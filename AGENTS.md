...
`required_paths` in `tools/sparkle/VERSION` is the enforcement list, checked by `verify_sparkle_vendor` in `scripts/build-dmg.sh`, so `--build-only` is the fastest way to catch a bad vendor.
Upgrade only via BUILD.md's "Upgrading the Sparkle framework" — it carries the full procedure including the framework-root symlinks that a `Versions/B/`-only copy drops.

To debug a nil installer launcher, probe `Bundle(path: ".../Sparkle.framework").url(forAuxiliaryExecutable: "Updater.app")` in a one-shot Swift script.
Nil means the framework-root `Updater.app` symlink is missing or the bundle root is wrong.
...
`todos.md` is active-work only.
Do not re-add compatibility risk registers, review archives, or "pruned for over-engineering" parking lots; completed analysis belongs in git history.
