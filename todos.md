...
- [ ] **`WindowChip` vs `WindowGroupChip`: ~90 duplicated lines.** `WindowChips.swift` — `chipFill`/`chipChrome`/`borderOpacity`/`topHighlightOpacity`/frontmost accent/hover are byte-identical between the two chip structs. Extract a shared `ChipChrome` modifier. Align the stray `NotificationBadge` offset (one uses `x:4`, the other `x:6`). Deferred: pure visual code — diff a before/after render.
...
- [ ] **Minor consistency.** `BehaviorTab` defaults `settings` inline (`SettingsView.swift:71`) unlike other tabs. `MenuBarController` sets menu title/image in both Combine sinks and `menuNeedsUpdate` (redundant). `CollapsedTabPosition`/`CollapsedTabPlacement` duplicate only the two-case list (mapping is already single-source in Core). `collapsedTabWidth = 56` duplicated between `BarController.swift:17` and `CollapsedTabMetrics.clusterWidth`. `LauncherController.resignObserver` never removed (singleton-safe).
